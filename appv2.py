...
form.title("Hui Code Gui v2")

form.title("Please configure parameters:")

file = ""  # form.file_uploader("Select a job rules file")

file2 = form.file_uploader("Please select a Resource Count file filtered by resource")

date = form.date_input("Select a Date")

timee = form.time_input(
...
user_input = form.text_input("Productivity Factor to use", "0.657534246575342")
...
if submit_button:
    ...
    if file2:
        ...
        filename2 = ""
    # form.text(f"The file selected for rules:{filename}")

    form.text(f"The Resource Count is:{filename2}")
    fdate = date.strftime("%m/%d/%y")
    timee = timee.strftime("%H:%M")
    form.text(f"We will start processing the file at :{fdate} {timee}")
    ...
    actual_shift_hours = [
        ai - bi / 60 for ai, bi in zip(shift_hours, shift_early_release)
    ]
    shift = [
        ...
        for k in work_type:
            if circular == 1:  # if we consider circular demand over periodical times
                ...
                if (
                    ...
                    ts = day_time[
                        (workforce_indx[t] - type_shift[k] + 1) : (
                            workforce_indx[t] + 1
                        )
                    ]
                    tsb = day_time[
                ...
                if (
                    ...
                    tsb = day_time[
                        max(workforce_indx[t] - type_shift[k] + 1, 0) : workforce_indx[
                            t
                        ]
                        + 1
                    ]
    ...
    for t in day_time:
        if circular == 1:
            ...
            tsa_U = day_time[
                workforce_indx[t]
                + 1 : min(workforce_indx[t] + w + 1, len(workforce_indx))
            ]
        tsa = tsa_L + tsa_U
        # print(t, tsa)
        ...
        if tsa != []:
            ...
            workforce_model.dmd.add(
                workforce_model.y[t] <= 0 + M * workforce_model.ya[t]
            )
            for ta in tsa:
                workforce_model.dmd.add(
    ...
    workforce_model.TotalCost = Objective(
        expr=sum(
            work_salary[k] * workforce_model.n[k, t]
            for k in work_type
            for t in day_time
        )
        * switch_salary
        ...
        * sum(
        ...
        + NH_opt
        * sum(
            sum(workforce_model.n[k, t] for t in day_time) / productivity
            for k in work_type
        ),
        sense=minimize,
    )
    ...
    for t in day_time:
        ...
        for k in work_type:
            if workforce_model.n[k, t]() != 0.0:
                string = (str(workforce_model.n[k, t]), "=", workforce_model.n[k, t]())
                st.write(string)

    string = ("Objective value is: ", workforce_model.TotalCost())
    st.write(string)
    ...
    st.download_button("download", csvfile.getvalue(), file_name="huisultados.csv")

    # creating the dataset
    data = workforce_demand_daily
    times = list(data.keys())
    workers_n = list(data.values())

    CumuActStaff = [0.0] * len(
        day_time
    )  # To be expanded to [[0.0]*len(times_slot)]* len(work_type)
    j = 0
    for t in day_time:
        # ts = day_time[max(workforce_indx[t]-type_shift[k],0):workforce_indx[t]+1 for k in work_type]
        CumuStaff = 0
        for k in work_type:
            if circular == 1:
                # Estimate the index of the time slots whose workers still have their shift active
                if workforce_indx[t] - type_shift[k] + 1 < 0:
                    sft = (workforce_indx[t] - type_shift[k] + 1) % len(workforce_indx)
                    shifted_time = getRangeList(day_time, sft)
                    ts = shifted_time[0 : type_shift[k]]
                else:
                    ts = day_time[
                        (workforce_indx[t] - type_shift[k] + 1) : (
                ...
                # If non-circular, then we consider the following time:
                ts = day_time[
                    max(workforce_indx[t] - type_shift[k] + 1, 0) : workforce_indx[t]
                    + 1
                ]
            ...
            CumuStaff += sum(workforce_model.n[k, t]() for t in ts)
        CumuActStaff[j] = CumuStaff
        j += 1

    fig = plt.figure(figsize=(300, 25))

    # creating the bar plot
    plt.bar(times, workers_n, color="blue", width=0.4)

    color_style = ["pink", "olive", "purple", "orange", "red", "cyan", "green", "grey"]
    type_color = dict(zip(work_type, color_style))
    ...
    for tp in work_type:
        plt.bar(
            times,
            (workforce_model.n[tp, :]()),
            # facecolor="blue", # The fill color
            color=type_color[tp],  # The outline color
            alpha=0.2,
        )  # Transparency of the fill
        wk = workforce_model.n[tp, :]()
        for i in range(len(wk)):
            plt.text(x=times[i], y=wk[i], s=wk[i], size=20)
    ...
    plt.xlabel("Time in a day (hh:mm)")
    plt.ylabel("Workforce")
    plt.title("Workforce scheduling")

    type_color.update({"Requirement": "Blue"})

    labels = list(type_color.keys())
    handles = [
        plt.Rectangle((0, 0), 150, 150, color=type_color[label]) for label in labels
    ]
    plt.legend(handles, labels, loc="upper left")

    for i in range(len(workers_n)):
        plt.text(x=times[i], y=workers_n[i], s=workers_n[i], size=20)
        plt.text(x=times[i], y=CumuActStaff[i], s=CumuActStaff[i], size=20)
